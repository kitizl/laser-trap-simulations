...
float feedback(float x, float v, float fb_gain){
	return 1 + fb_gain*x*v;
}

float PLL(float in_signal, float Kp, float Ki, float Kd){
	float out_signal = 0.0;
	return out_signal;
}

int main(int argc, char *argv[]){
	 // A C++ Program specifically dedicated to simulating Parametric Feedback Cooling
    ...
    */
	...
	float fb_gain = std::stof(argv[6]); // the "gain" of the parametric feedback signal
    ...
    for(int i = 1; i < Nsteps-1; i++){
        ...
        a = -omega*omega*feedback(x, v, fb_gain)*x - gamma*v + noise_force;
